...
function getFirstResolvedPromiseResult(promises) {
  const neverRejectPromises = promises.map((promise) =>
    promise.catch(() => Symbol.for('reject'))
  );

  return Promise.race(neverRejectPromises).then((result) => {
    if (result === Symbol.for('reject')) {
...
function getAllResult(promises) {
  const resolvedPromises = promises.map((promise) => promise.catch(() => null));
